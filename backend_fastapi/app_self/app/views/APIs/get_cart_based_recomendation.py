...
from modules import transform_prods_to_dict, content_based_just_pandas, is_in_storedb, recommended_for_user_cart_based, read_user_cart_recs_from_csv
...
@router.get('/get_cart_based_recomendation/')
async def view(request: Request, bg_tasks: BackgroundTasks = None):
    ...
    if prods_id:
        ...
        selected_prods = df[df['Product ID'].isin(prods_id)]
        prods_data = transform_prods_to_dict.transform(selected_prods)
        print('xd?')

        return {'prods_data' : prods_data}
