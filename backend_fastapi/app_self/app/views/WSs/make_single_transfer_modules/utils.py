                ...
                if client_response.get('received_chunk'):
                    ...
                    if WAIT_FOR_CLIENT_RESPONSE_WHILE_UPLOADING:
                        try:
                            ...
                            # HOST_WS is dead — host disconnected, not client
                            print('host disconnected mid-transfer, notifying client')
                            # raise RuntimeError
        ...
        while True:
            try:
                if self.THIS_ROOM_DATA.get('client_ready'):
                    ...
                    await self.HOST_WS.send_json({'begin_upload':True, 'file_id':self.FILE_ID_ASSIGNED_BY_USER, 'client_username': self.THIS_ROOM_DATA['client_username']}) if not HOST_NOTIFIED else None
                    HOST_NOTIFIED = True
